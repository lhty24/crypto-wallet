...
use crypto_wallet_backend::core::wallet::{
    derivation, generate_test_mnemonic, Account, Chain, EntropyLevel, HDWallet, MnemonicManager,
};
...
fn test_complete_hd_wallet_workflow() {
    ...
    for account in &accounts {
        ...
        println!(
            ...
            account.private_key_bytes().len()
        );
        println!(
            "   Public key length: {} bytes",
            account.public_key_bytes().len()
        );
    }
...
fn test_multi_chain_account_generation() {
    ...
    for chain in chains {
        ...
        let expected_path = format!("m/44'/{}'/{}'/{}/{}", chain.coin_type(), 0, 0, 0);
...
fn test_multiple_accounts_per_chain() {
    ...
    for i in 0..eth_accounts.len() {
        for j in (i + 1)..eth_accounts.len() {
            ...
            assert_ne!(
                eth_accounts[i].private_key_bytes(),
                eth_accounts[j].private_key_bytes()
            );
        }
    }
...
fn test_derivation_path_utilities() {
    ...
    assert_eq!(
        derivation::parse_coin_type_from_path("m/44'/60'/0'/0/0"),
        Some(60)
    );
    assert_eq!(
        derivation::parse_coin_type_from_path("m/44'/501'/0'/0/0"),
        Some(501)
    );
