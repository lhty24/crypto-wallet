...
use crypto_wallet_backend::core::security::{
    EncryptionManager, EncryptedWallet, SecurityConfig, WalletMetadata,
    SecureString, PasswordStrengthLevel, utils
};
use crypto_wallet_backend::core::wallet::{MnemonicManager, EntropyLevel};
use std::fs;
...
fn test_complete_encryption_workflow() {
    ...
    println!("✅ Generated test mnemonic with {} words", mnemonic.word_count());
    ...
    let encrypted = encryption_manager.encrypt_mnemonic(&mnemonic_phrase, password).unwrap();
    ...
    let decrypted = encryption_manager.decrypt_mnemonic(&encrypted, password).unwrap();
...
fn test_all_security_levels() {
    ...
    for (name, config) in configs {
        ...
        println!("   Memory cost: {} KiB, Time cost: {}", 
...
fn test_password_security_features() {
    ...
    let encrypted = manager.encrypt_mnemonic(test_data, correct_password).unwrap();
    
    // Test correct password
    assert!(manager.verify_password(&encrypted, correct_password));
    let decrypted = manager.decrypt_mnemonic(&encrypted, correct_password).unwrap();
    ...
    let re_encrypted = manager.change_password(&encrypted, correct_password, new_password).unwrap();
    ...
    let final_decrypted = manager.decrypt_mnemonic(&re_encrypted, new_password).unwrap();
...
fn test_wallet_metadata_and_info() {
    ...
    let metadata = WalletMetadata {
        ...
        supported_chains: vec!["Bitcoin".to_string(), "Ethereum".to_string(), "Solana".to_string()],
        last_backup_reminder: Some(1234567890),
    };
    
    let encrypted = manager.encrypt_mnemonic_with_metadata(test_mnemonic, password, metadata.clone()).unwrap();
    ...
    assert_eq!(info.metadata.description, Some("A wallet for testing encryption".to_string()));
...
fn test_password_strength_analysis() {
    ...
    let test_cases = [
        ...
        ("Password123", PasswordStrengthLevel::Medium, "good mix but common base"),
        ("MyStr0ngP@ssw0rd!", PasswordStrengthLevel::Strong, "strong with symbols"),
        ("SuperComplexP@ssw0rd123!@#", PasswordStrengthLevel::VeryStrong, "very strong and long"),
    ];
    
    for (password, expected_level, description) in test_cases {
        let strength = utils::check_password_strength(password);
        
        println!("📝 Password: '{}' - {} (Score: {})", password, description, strength.score);
        println!("   Level: {:?}, Expected: {:?}", strength.level, expected_level);
...
fn test_encryption_performance() {
    ...
    for i in 0..iterations {
        ...
        let encrypted = dev_manager.encrypt_mnemonic(test_mnemonic, password).unwrap();
        ...
        println!("   Iteration {}: Encrypt {:?}, Decrypt {:?}", 
            i + 1, encrypt_time, decrypt_time);
    }
    ...
    assert!(avg_encrypt.as_millis() < 1000, "Encryption should be under 1 second");
    assert!(avg_decrypt.as_millis() < 1000, "Decryption should be under 1 second");
...
fn test_encryption_with_hd_wallet_integration() {
    ...
    let recovered_mnemonic = mnemonic_manager.parse(recovered_mnemonic_string.as_str()).unwrap();
    ...
    use crypto_wallet_backend::core::wallet::{HDWallet, Chain};
    ...
    for chain in [Chain::Bitcoin, Chain::Ethereum, Chain::Solana] {
        ...
        assert_eq!(original_account.private_key_bytes(), recovered_account.private_key_bytes());
        
        println!("   ✅ {} addresses match: {}", 
            chain.name(), original_account.address());
    }
...
fn test_real_world_wallet_backup_scenario() {
    ...
    println!("   Password strength: {:?} (Score: {})", 
    ...
    let backup = EncryptedWallet::encrypt_mnemonic_high_security(&user_mnemonic.phrase(), user_password).unwrap();
    ...
    let recovered_mnemonic = mnemonic_manager.parse(recovered_mnemonic_str.as_str()).unwrap();
    ...
    for chain in [Chain::Bitcoin, Chain::Ethereum, Chain::Solana] {
        let original_addr = original_hd_wallet.derive_account(chain, 0).unwrap().address().to_string();
        let recovered_addr = recovered_hd_wallet.derive_account(chain, 0).unwrap().address().to_string();
