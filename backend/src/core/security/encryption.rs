...
impl EncryptionManager {
    ...
    pub fn decrypt_mnemonic(
        ...
        password: &str,
    ) -> Result<SecureString, EncryptionError> {
    ...
    fn derive_key_from_password(
        ...
        let hash_bytes = password_hash.hash.ok_or_else(|| {
            EncryptionError::PasswordHashingFailed("No hash produced".to_string())
        })?;

        if hash_bytes.len() != 32 {
            return Err(EncryptionError::PasswordHashingFailed(format!(
    ...
    fn encrypt_with_key(
        ...
        if encrypted.len() < 16 {
            return Err(EncryptionError::AesGcmError(
...
impl EncryptedWallet {
    ...
    pub fn encrypt_mnemonic_high_security(
        mnemonic: &str,
        password: &str,
