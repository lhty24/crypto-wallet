...
impl HDWallet {
    ...
    pub fn derive_account(
        ...
        chain: Chain,
        account_index: u32,
    ) -> Result<Account, HDWalletError> {
        match chain {
            Chain::Bitcoin | Chain::Ethereum => self.derive_secp256k1_account(chain, account_index),
            Chain::Solana => self.derive_ed25519_account(chain, account_index),
        }
    }
    ...
    pub fn derive_multiple_accounts(
        ...
        chain: Chain,
        count: u32,
    ) -> Result<Vec<Account>, HDWalletError> {
...
mod tests {
    use super::*;
    use crate::core::wallet::mnemonic::{EntropyLevel, MnemonicManager};
